...
class CompletionCallback(ABC):
    def __init__(self, config: DictConfig, scheduler: "ChatCompletionScheduler"):
        ...
        logger.warning(f"Initialized tools: {self.tools}")
...
class ToolCompletionCallback(CompletionCallback):
    def __init__(self, config: DictConfig, scheduler: "ChatCompletionScheduler"):
        ...
        print("ToolCompletionCallback __init__() invoked")

        # TODO: add reward manager to calculate reward score once a sample finish

    async def __call__(self, messages: List[Dict[str, str]], completions: ChatCompletion, info: Dict[str, Any]):
        # print("ToolCompletionCallback.__call__() invoked")
        ...
        try:
            ...
            if len(tool_calls) == 0:
                print("Incorrect tool call (parsing)")
                return
        except:
            print("Incorrect tool call (parsing)")
            return
        ...
        try:
            for tool_call in tool_calls:
                # Update tool_call.function.arguments with info.__sampling_params__.tool_kwargs
                # print("info['__tools_kwargs__']:", info["__tools_kwargs__"])
                tools_kwargs = info["__tools_kwargs__"]
                tasks.append(self._call_tool(tool_call, tools_kwargs)) # tools_kwargs: problem-specific env params for tools, not generated by LLM
            tool_responses = await asyncio.gather(*tasks)
        except:
            print("Incorrect tool call (execute)")
            return
    ...
    async def _call_tool(self, tool_call, tools_kwargs) -> Dict[str, str]:
        ...
        try:
            tool_response, tool_reward_score, tool_metrics = await tool.execute(instance_id, tool_args, tools_kwargs=tools_kwargs.get(tool_name, {}))
...
class ChatCompletionScheduler:
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        # print("def generate_sequences() - batch param:", batch)
        ...
        # print(f"batch.non_tensor_batch.keys():", batch.non_tensor_batch.keys(), flush=True)
        # print(batch.non_tensor_batch["tools_kwargs"][0])
        # print("completion callback:", self.completion_callback.__class__)
        ...
        tools_kwargs_batched = batch.non_tensor_batch["tools_kwargs"].repeat(n, axis=0)
        for batch_index, conversation in enumerate(batch.non_tensor_batch["raw_prompt"].repeat(n, axis=0)):
            ...
            # Monkkey patch system prompt
            # batch_conversations[batch_index][0]["content"] = "Always wrap your final answer in <answer></answer> tags. It should only include the program code, and not other additives like backticks (```).\n\n" + batch_conversations[batch_index][0]["content"]
            ...
            tasks.append(
                asyncio.create_task(
                    self._submit_chat_completions_semaphore(
                        ...
                        tools_kwargs=tools_kwargs_batched[batch_index]
                    )
                )
            )
    ...
    async def _submit_chat_completions_semaphore(self, messages: List[Dict[str, str]], request_id: str, sampling_params: Dict[str, Any], tools_kwargs: Dict[str, Any]):
        done = asyncio.Event()
